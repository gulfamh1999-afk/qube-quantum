...
class QubeEngine:
    ...
    def _build_ansatz(self, angles, params):
        ...
        for i, theta in enumerate(angles):
            ...
            qc.ry(params[i], i)
            qc.rz(params[i + n], i)
        for i in range(n - 1):          # ← FIXED: full nearest-neighbor chain (matches paper)
            qc.cx(i, i + 1)
    ...
    def train(self, dataset, maxiter=120):
        ...
        return result.x, result.fun   # ← FIXED: now returns (params, loss) like the sim file
...
if __name__ == "__main__":
    ...
    params, loss = local_engine.train(dataset)   # ← now matches the new train() signature
